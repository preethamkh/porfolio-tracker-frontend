...
export default function SecuritySearchCombobox({
    ...
    useEffect(() => {
        // Guard clause: Don't search if query is too short
        if (query.length < 2) {
        ...
        const timer = setTimeout(async () => {
            ...
            try {
                ...
                const mockResults: SecuritySearchResult[] = [
                    {
                        id: '1',
                        symbol: 'AAPL',
                        name: 'Apple Inc.',
                ...
                <div className="absolute z-50 w-full mt-1 bg-white border border-gray-200 rounded-lg shadow-lg max-h-64 overflow-auto">
                    {results.map((security, index) => (
                        <button
                            ...
                            className={`w-full px-4 py-3 text-left hover:bg-gray-50 transition-colors border-b border-gray-100 last:border-b-0 ${index === selectedIndex ? 'bg-blue-50' : ''
                                }`}
                        >
